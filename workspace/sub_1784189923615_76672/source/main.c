...
// --- USER CODE ---
int maxDepth(struct TreeNode* root) { if(!root) return 0; int l = maxDepth(root->left); int r = maxDepth(root->right); return 1 + (l > r ? l : r); }
...
int main() {
    static char line0[1048576];
        ...
        struct TreeNode* root = deserializeTree(line0);
