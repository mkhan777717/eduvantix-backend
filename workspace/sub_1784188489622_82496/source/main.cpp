...
// --- USER CODE ---
class Solution { public: ListNode* reverseList(ListNode* head) { ListNode* prev=nullptr; while(head) { ListNode* nxt=head->next; head->next=prev; prev=head; head=nxt; } return prev; } };
...
int main() {
    string line0;
        ...
        ListNode* head = deserializeList(line0);
    Solution solver;
        ListNode* result = solver.reverseList(head);
    cout << serializeList(result) << endl;
    freeList(head);
    freeList(result);
    return 0;
}
