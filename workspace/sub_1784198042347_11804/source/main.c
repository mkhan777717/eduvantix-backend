...
struct Node* dfs(struct Node* node) {
    ...
    struct Node* existing = find(node);
    if (existing)
        return existing;
    ...
    map[size].original = node;
    map[size].clone = clone;
    size++;
...
struct Node* solve(struct Node* node) {
    size = 0;
    return dfs(node);
}
